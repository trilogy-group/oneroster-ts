  ...
  implements SDKInitHook, BeforeRequestHook, AfterErrorHook
{
  ...
  private async getCredentialsGlobal(
    ...
    const envTokenURL = env().ONEROSTER_TOKEN_URL ?? DEFAULT_TOKEN_URL;
    ...
    return {
      clientID: out?.clientID ?? env().ONEROSTER_CLIENT_ID ?? "",
      clientSecret: out?.clientSecret ?? env().ONEROSTER_CLIENT_SECRET ?? "",
