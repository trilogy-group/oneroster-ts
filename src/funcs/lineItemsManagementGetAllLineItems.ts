 ...
 */
export function lineItemsManagementGetAllLineItems(
  ...
  PageIterator<
    Result<
      operations.GetAllLineItemsResponse,
...
> {
  const parsed = safeParse(
    request,
    (value) => operations.GetAllLineItemsRequest$outboundSchema.parse(value),
  ...
  const path = pathToFunc("/ims/oneroster/gradebook/v1p2/lineItems/")();
  ...
  const [result, raw] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, operations.GetAllLineItemsResponse$inboundSchema, {
      key: "Result",
    }),
  ...
  const nextFunc = (
    ...
    const results = dlv(responseData, "lineItems");
