 ...
 */
export function schoolsManagementGetTeachersForClassInSchool(
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.GetTeachersForClassInSchoolRequest$outboundSchema.parse(value),
  ...
  const pathParams = {
    ...
    schoolSourcedId: encodeSimple("schoolSourcedId", payload.schoolSourcedId, {
      explode: false,
      charEncoding: "percent",
    }),
  };
  ...
  const [result, raw] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, operations.GetTeachersForClassInSchoolResponse$inboundSchema, {
      key: "Result",
    }),
  ...
  const nextFunc = (
    ...
    const results = dlv(responseData, "users");
