 ...
 */
export function resourcesManagementGetResourcesForUser(
  ...
  PageIterator<
    Result<
      operations.GetResourcesForUserResponse,
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.GetResourcesForUserRequest$outboundSchema.parse(value),
  ...
  const pathParams = {
    userSourcedId: encodeSimple("userSourcedId", payload.userSourcedId, {
      explode: false,
  ...
  const [result, raw] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, operations.GetResourcesForUserResponse$inboundSchema, {
      key: "Result",
    }),
  ...
  const nextFunc = (
    ...
    const results = dlv(responseData, "resources");
