 ...
 */
export function coursesManagementGetCourse(
  client: OneRosterCore,
  request: operations.GetCourseRequest,
  options?: RequestOptions,
): APIPromise<
  Result<
    operations.GetCourseResponse,
...
> {
  const parsed = safeParse(
    request,
    (value) => operations.GetCourseRequest$outboundSchema.parse(value),
  ...
  const path = pathToFunc("/ims/oneroster/rostering/v1p2/courses/{sourcedId}")(
    pathParams,
  );
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, operations.GetCourseResponse$inboundSchema),
