 ...
 */
export function teachersManagementGetTeacher(
  client: OneRosterCore,
  request: operations.GetTeacherRequest,
  options?: RequestOptions,
): APIPromise<
  Result<
    operations.GetTeacherResponse,
...
> {
  const parsed = safeParse(
    request,
    (value) => operations.GetTeacherRequest$outboundSchema.parse(value),
  ...
  const path = pathToFunc("/ims/oneroster/rostering/v1p2/teachers/{sourcedId}")(
    pathParams,
  );
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, operations.GetTeacherResponse$inboundSchema),
