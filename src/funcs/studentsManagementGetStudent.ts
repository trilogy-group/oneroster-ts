 ...
 */
export function studentsManagementGetStudent(
  client: OneRosterCore,
  request: operations.GetStudentRequest,
  options?: RequestOptions,
): APIPromise<
  Result<
    operations.GetStudentResponse,
...
> {
  const parsed = safeParse(
    request,
    (value) => operations.GetStudentRequest$outboundSchema.parse(value),
  ...
  const path = pathToFunc("/ims/oneroster/rostering/v1p2/students/{sourcedId}")(
    pathParams,
  );
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, operations.GetStudentResponse$inboundSchema),
