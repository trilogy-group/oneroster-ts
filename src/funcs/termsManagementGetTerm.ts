 ...
 */
export function termsManagementGetTerm(
  client: OneRosterCore,
  request: operations.GetTermRequest,
  options?: RequestOptions,
): APIPromise<
  Result<
    operations.GetTermResponse,
...
> {
  const parsed = safeParse(
    request,
    (value) => operations.GetTermRequest$outboundSchema.parse(value),
  ...
  const path = pathToFunc("/ims/oneroster/rostering/v1p2/terms/{sourcedId}")(
    pathParams,
  );
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, operations.GetTermResponse$inboundSchema),
