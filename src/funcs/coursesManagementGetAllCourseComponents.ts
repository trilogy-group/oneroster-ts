 ...
 */
export function coursesManagementGetAllCourseComponents(
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.GetAllCourseComponentsRequest$outboundSchema.parse(value),
  ...
  const path = pathToFunc("/ims/oneroster/rostering/v1p2/courses/components")();
  ...
  const [result, raw] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, operations.GetAllCourseComponentsResponse$inboundSchema, {
      key: "Result",
    }),
  ...
  const nextFunc = (
    ...
    const results = dlv(responseData, "courseComponents");
